...
static uint8_t gps_sv_count = 0;   // from $GPGSV field 3 (GPS constellation)
static uint8_t glo_sv_count = 0;   // from $GLGSV field 3 (GLONASS constellation)
 ...
 * The CAM-M8 outputs one GNGSA per active constellation per second. */
 ...
 * GNGSA field layout:
 *   0  $GNGSA
 *   1  A/M        Auto/manual mode selection
 *   2  1/2/3      Fix type: 1 = no fix, 2 = 2D, 3 = 3D
 *   3–14          PRN numbers of satellites used (empty = unused slot)
 *   15  PDOP      (not used here)
 *   16  HDOP
 *   17  VDOP
 *
 * gsa_reset_pending ensures we accumulate sats_in_use across all GNGSA
 * sentences in one cycle before the next RMC resets the counter. */
 ...
 * GSV field layout:
 *   0  $GPGSV / $GLGSV
 *   1  Total messages this cycle
 *   2  This message number (1-based)
 *   3  Total satellites in view for this constellation
 *
 * Field 3 is the same on every message in the sequence so we can read it
 * from whichever sentence arrives without tracking sequence numbers. */
static void parse_gsv_fast(const char *line)
{
    ...
    if      (line[2] == 'P') gps_sv_count = count;
    else if (line[2] == 'L') glo_sv_count = count;
...
void GPS_push_line(const char *line, int length)
{
    ...
    if (memcmp(line, "$GNRMC", 6) == 0) {
         ...
         * Also mark that the next GSA sentence should reset sats_in_use,
         * starting a fresh count for the new NMEA cycle. */
        GPS_RMC_t fix;
        ...
        /* Satellites in view per constellation — updates sats_in_view */
        parse_gsv_fast(line);
    }
}
